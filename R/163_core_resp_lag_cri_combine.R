corr_resp_lag_combine = function(age_class="80+",chains=1:4,mod="mod8",save.date){
  ...
  n_iter=max(cum_excess_pand_df$iter,na.rm=TRUE)
  cum_excess_pand_df = 
    rbind(cum_excess_pand_df %>% filter(cod_group!="COVID-19"),
          cum_excess_pand_df %>% filter(cod_group=="COVID-19") %>% dplyr::select(-iter) %>% 
            cross_join(data.frame(iter=1:n_iter))) %>% 
    arrange(iter)
  ...
  reshaped_df <- cum_excess_pand_df %>%
    ...
    dplyr::select(week.id,age_class,cod_group,iter,excess) %>% 
