aggregate_fit_stan = function(data_fit, fit, cmdstan=TRUE){
  #get sample of deaths
  if(cmdstan){
    d=fit$draws()[,,]
    n_iter_per_chain = d[,1,1] %>% length()
    deaths_pred_sample = data_fit %>%
      dplyr::mutate(data_row= dplyr::row_number()) %>% 
      dplyr::select(cal_year,cal_week,age_class,sex,data_row) %>% 
      left_join(.,
                as.data.frame(ftable(d[,,grepl("deaths_pred",dimnames(d)[[3]])])) %>% 
    ...
    deaths_pred_sample = data_fit %>%
      ...
      dplyr::select(cal_year,cal_week,age_class,sex,data_row) %>% 
      left_join(.,
                rstan::extract(fit,pars="deaths_pred") %>% 
                  as.data.frame() %>% 
                  dplyr::mutate(iter = dplyr::row_number()) %>% 
                  pivot_longer(cols=contains("deaths_pred"),names_to = "var",values_to="values") %>%
                  separate(col=var,into=c("variable","data_row"),sep="\\.") %>% 
                  dplyr::mutate(data_row=as.numeric(data_row)),by="data_row")
  }
  
  #aggregate deaths by cal_year, cal_week and age_class (i.e. over sex)
  deaths_pred_agg = deaths_pred_sample %>% 
    group_by(cal_year,cal_week,age_class,iter) %>%
  ...
  d = data_fit %>%
    group_by(cal_year,date,cal_week,age_class) %>% 
    dplyr::summarise(n=sum(n),.groups="drop") %>% 
    left_join(deaths_pred_agg, by=c("cal_year","cal_week","age_class"))
