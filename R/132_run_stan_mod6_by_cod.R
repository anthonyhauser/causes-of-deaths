run_stan_mod6_by_cod = function(cod_agg_pop_df, age_class, run.model=TRUE,
                                reg_var = c("sex"),reg_var_ref=c("M"),
                                save.date){
  #name to save
  if(file.exists(paste0(code_root_path,"results/",save.date,"/mod6_stan_diag_",age_class,".RDS"))){
    stan_diag = readRDS(paste0(code_root_path,"results/",save.date,"/mod6_stan_diag_",age_class,".RDS"))
    print(stan_diag)
    return(NULL)
  }
  ...
  mod6_cmdstan <- cmdstan_model(paste0(code_root_path,"stan/mod6_GP_year_season_causes.stan"))
  #mod6_cmdstan_0 <- cmdstan_model(paste0(code_root_path,"stan/mod6_GP_year_season_causes0.stan"))
  ...
  if(run.model){
    ...
    if(FALSE){
      library(rstan)
      mod6_rstan <- stan_model(paste0(code_root_path,"stan/mod6_GP_year_season_causes.stan"))
      fit6_rstan <- rstan::sampling(
        mod6_rstan,
    ...
    chains = fit6$time()$chains %>% filter(!is.na(total)) %>% pull(chain_id) %>% .[1:4]
  
    stan_diag =  data.frame(time = fit6$time()$chains %>% filter(!is.na(total)) %>% pull(total) %>% max(),
                            num_successful_chains =  fit6$diagnostic_summary()$num_divergent %>% length(),
                            num_divergent = fit6$diagnostic_summary()$num_divergent %>% sum(),#fit$sampler_diagnostics()
                            num_max_treedepth = fit6$diagnostic_summary()$num_max_treedepth %>% sum(),
                            ebfmi = fit6$diagnostic_summary()$ebfmi %>% min(),
                            rhat = fit6$summary() %>% filter(!is.na(rhat)) %>% pull(rhat) %>% max()) %>% 
  ...
  #divergence
  print(paste(c("divergence: ",fit6$sampler_diagnostics()[,,"divergent__"] %>% apply(2,sum)),sep=" "))
  ...
  if(!file.exists(paste0(code_root_path,"results/",save.date,"/mod6_stan_diag_",age_class,".RDS")) &
    ...
    #by week
    t0=Sys.time()
    data_pred_week_cause = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
    ...
    data_pred_week = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
                                    groups=c("cal_year","cal_week","age_class","date"),chains=chains) %>% 
      left_join(stan_diag %>% dplyr::select(age_class, is.stan.ok),by=c("age_class"))
    t1=Sys.time()
    #by COVID-19 phase
    data_pred_phase_cause = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
                                           groups=c("covid_phase","age_class","cod_group"),chains=chains) %>% 
      left_join(stan_diag %>% dplyr::select(age_class, is.stan.ok),by=c("age_class"))
    data_pred_phase = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
                                     groups=c("covid_phase","age_class"),chains=chains) %>% 
      left_join(stan_diag %>% dplyr::select(age_class, is.stan.ok),by=c("age_class"))
    t2=Sys.time()
    #by year
    data_pred_year_cause = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
    ...
    data_pred_year = aggregate_stan_mod6(data, fit6, cmdstan=TRUE,
    ...
    ###########################################################################
    #sex
    # reg_effect = fit6$summary(variables = c("beta_reg"), "mean",~quantile(.x, probs = c(0.025, 0.975))) %>% 
    ...
    #Correlation
    sigma = fit6$summary(variables = c("sigma"), "mean",~quantile(.x, probs = c(0.025, 0.975))) %>% 
      #summary_cmdstanr(fit=fit6, variable = "sigma",chains=chains) %>% #needed if we wants to select some chains (successful chains)
      ...
      left_join(data %>% dplyr::select(cod_group,cod_group_id) %>% unique(),by=c("cod_group_id"))
    Sigma_mat = fit6$summary(variables = c("Sigma"), "mean",~quantile(.x, probs = c(0.025, 0.975))) %>% 
      #summary_cmdstanr(fit=fit6, variable = "Sigma",chains=chains) %>% 
      ...
      left_join(data %>% dplyr::select(cod_group,cod_group_id) %>% unique(),by=c("cod_group_id")) %>% 
      left_join(data %>% dplyr::select(cod_group2=cod_group,cod_group_id2=cod_group_id) %>% unique(),by=c("cod_group_id2"))
    
    #GP
    #periodic
    week_GP = fit6$summary(variables = c("f_week"), "mean",~quantile(.x, probs = c(0.025, 0.975))) %>% 
      #summary_cmdstanr(fit=fit6, variable = "f_week",chains=chains) %>% 
    ...
    #long term trend
    year_GP = fit6$summary(variables = c("f_year"), "mean",~quantile(.x, probs = c(0.025, 0.975))) %>% 
      #summary_cmdstanr(fit=fit6, variable = "f_year",chains=chains) %>% 
      tidyr::extract(variable,into=c("var","cod_group_id","week.id"),
                     regex =paste0('(\\w.*)\\[',paste(rep("(.*)",2),collapse='\\,'),'\\]'), remove = T) %>% 
      as_tibble() %>% 
      dplyr::select(est=mean,lwb=`2.5%`,upb=`97.5%`,week.id,cod_group_id) %>%
    ...
    #save
    if(!file.exists(paste0(code_root_path,"results/",save.date,"/mod6_stan_diag_",age_class,".RDS"))){#check again in case another node in the cluster save results
      print("Save")
      saveRDS(stan_diag, file=paste0(code_root_path,"results/",save.date,"/mod6_stan_diag_",age_class,".RDS"))
      #saveRDS(reg_effect, file=paste0(code_root_path,"results/",save.date,"/mod6_reg_effect_",age_class,".RDS"))
      
      saveRDS(data_pred_week, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_week_",age_class,".RDS"))
      saveRDS(data_pred_phase, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_phase_",age_class,".RDS"))
      saveRDS(data_pred_year, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_year_",age_class,".RDS"))
      
      saveRDS(data_pred_week_cause, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_week_cause_",age_class,".RDS"))
      saveRDS(data_pred_phase_cause, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_phase_cause_",age_class,".RDS"))
      saveRDS(data_pred_year_cause, file=paste0(code_root_path,"results/",save.date,"/mod6_data_pred_year_cause_",age_class,".RDS"))
      
      saveRDS(sigma, file=paste0(code_root_path,"results/",save.date,"/mod6_sigma_",age_class,".RDS"))
      saveRDS(Sigma_mat, file=paste0(code_root_path,"results/",save.date,"/mod6_Sigma_mat_",age_class,".RDS"))
      
      saveRDS(year_GP, file=paste0(code_root_path,"results/",save.date,"/mod6_year_GP_",age_class,".RDS"))
      saveRDS(week_GP, file=paste0(code_root_path,"results/",save.date,"/mod6_week_GP_",age_class,".RDS"))
    }
  }
