...
# cancer_deaths_plot = function(cod_ind_df, n_week_agg = 1, k=9){
#   ##############################################################################
#   #ICD10 block 2/3 for cancer
#   #load icd10 table for cancer
#   icd10_df = read_excel(paste0(data_folder,"icd10_icd11_mapping.xlsx"))
...
#     scale_x_date(name="",date_labels = "%b %y")+
#     scale_y_continuous(name="Number of individuals\ndying with cancer")+
#     scale_color_manual(name="Cause of death",values=c("blue","violet","black","violet"),
#                        breaks = c(0,1,2,3),
#                        labels = c("No cancer","Cancer","Total","Expected"))+
#     scale_linetype_manual(name="Cause of death",values=c(1,1,1,3),
#                        breaks = c(0,1,2,3),
#                        labels = c("No cancer","Cancer","Total","Expected"))+
#     theme(legend.position = "bottom",
#           panel.spacing = unit(1.5, "lines"),
...
#  
#   if(FALSE){
#     #look at relative increase calculated as peak divided by minimum values during
#     d1 %>% filter(!cause_cancer,!is.na(n)) %>% 
#       group_by(age_class,all_cond) %>% 
#       dplyr::summarise(rel_increase=max(n)/min(n[mean_date<ymd("2021-01-01")]),
#                        rel_increase2=max(n)/mean(n[mean_date<ymd("2021-01-01")]))
#   }
#   
#   return(cowplot::plot_grid(p1,p2,p3,
#                    labels=c("A.","B.","C."),
