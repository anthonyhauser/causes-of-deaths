...
parameters {
  ...
  matrix<lower=0,upper=1>[N, 2] u_raw;
  cholesky_factor_corr[2] rho_chol;
}

model {
  matrix[N, 2] u;
  matrix[N, 2] Lb=uvar_bounds(pois_y1, pois_y2, lambda1, lambda2, 0);     // lower bound for uniform variates
  matrix[N, 2] Ub=uvar_bounds(pois_y1, pois_y2, lambda1, lambda2, 1);     // upper bound for uniform variates
  matrix[N, 2] Db=Ub-Lb;     // = Ub - Lb (useful for Jacobian)
  //---------------------------
  // Priors
  //---------------------------
