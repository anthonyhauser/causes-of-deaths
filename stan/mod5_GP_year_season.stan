...
parameters {
  vector[N_age] mu0;                           // Intercept
  array[N_age] vector[N_reg] beta_reg;             // Regression coefficients
  
  // GPs by age group
  vector[N_age] alpha_week;       // Weekly GP scale by age
  vector[N_age] lambda_week;      // Weekly GP lengthscale by age
  array[N_age] vector[2 * J_week] beta_week; // Basis coefficients for weekly GP
  
  vector[N_age] alpha_year;       // Yearly GP scale by age
  vector[N_age] lambda_year;      // Yearly GP lengthscale by age
  array[N_age] vector[M_year] beta_year; // Basis coefficients for yearly GP
}

transformed parameters {
  array[N_age] vector[2*J_week] diagSPD_week;
  array[N_age] vector[M_year] diagSPD_year;
  
  array[N_age] vector[N_x] f_week;
  array[N_age] vector[N_x] f_year;
  ...
  array[N] real mu;
  for(i in 1:N){
    mu[i] = mu0[age_id[i]] + X_reg[i] * beta_reg[age_id[i]] + f_week[age_id[i],week_id[i]] + f_year[age_id[i],week_id[i]] + log(n_pop[i]);
  }
}
...
generated quantities {
  ...
  vector[N_all] mu_all;
  for(i in 1:N_all){
    mu_all[i] = mu0[age_id_all[i]] + X_reg_all[i] * beta_reg[age_id_all[i]] + f_week[age_id_all[i],week_id_all[i]] +
                    f_year[age_id_all[i],week_id_all[i]] + log(n_pop_all[i]);
  ...
  vector[N_all] rel_excess = excess./deaths_all_pred;
}
