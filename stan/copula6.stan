functions {
  ...
  matrix uvar_bounds(array[,] int pois_y, vector lambda, vector phi,
                     int is_upper) {
    int N = size(pois_y);
    int J = dims(pois_y)[2];
    matrix[N, J] u_bounds;
      ...
      if (is_upper == 0) {
        for(j in 1:J){
          u_bounds[n, j] = pois_y[n,j] == 0.0
                          ? 0.0 :  neg_binomial_2_cdf(pois_y[n,j] - 1 | lambda[j], phi[j]);
        }
      } else {
        for(j in 1:J){
         u_bounds[n, j] =  neg_binomial_2_cdf(pois_y[n,j] | lambda[j], phi[j]);
        }
      }
    }
...
model {
  matrix[N, J] u;
  matrix[N, J] Lb=uvar_bounds(pois_y, lambda, phi, 0);     // lower bound for uniform variates
  matrix[N, J] Ub=uvar_bounds(pois_y, lambda, phi, 1);     // upper bound for uniform variates
