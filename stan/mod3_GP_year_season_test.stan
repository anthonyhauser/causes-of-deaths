...
transformed data {
  ...
  //real <lower=0> lambda_week=1;      // lengthscale of f
}
...
transformed parameters {
  ...
  array[N] real mu;
  for(i in 1:N){
    mu[i] = mu0 + Y_reg[i]+ f_year[week_id[i]] + f_week[week_id[i]] + log(n_pop[i]);
  }
}
...
generated quantities {
  ...
  for(i in 1:N_pand){
    mu_pand[i] = mu0 + Y_reg_pand[i] + f_week[week_id_pand[i]] + f_year[week_id_pand[i]] + log(n_pop_pand[i]);
