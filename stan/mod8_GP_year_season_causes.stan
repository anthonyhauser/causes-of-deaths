...
data {
  ...
  array[N_cause,N_sex,N_region] vector[N_all] deaths_all;
  array[N_cause,N_sex,N_region,N] int deaths;
  array[N_sex,N_region] vector[N_all] n_pop_all;
  array[N_sex,N_region] vector[N] n_pop;
...
parameters {
  ...
  array[N_cause] vector[N_sex-1] beta_sex0;             // Regression coefficients
  array[N_cause] vector[N_region-1] beta_region0;   
  ...
  vector<lower=0>[N_cause] sigma;
  array[N_cause] row_vector[N] eta;                   // Random effects
  cholesky_factor_corr[N_cause] rho_chol;
}
...
transformed parameters {
  ...
  matrix[N_cause,N] eta2 = rho_chol * to_matrix(eta);
  
  array[N_cause] vector[N_sex] beta_sex;
  array[N_cause] vector[N_region] beta_region;
  for(g in 1:N_cause){
    beta_sex[g] = append_row(0.0, beta_sex0[g]);
    beta_region[g] = append_row(0.0, beta_region0[g]);
  }
}
...
model {
  ...
  if(N_sex>1){
    for (g in 1:N_cause) {
      beta_sex0[g] ~ normal(0,1);
      beta_region0[g] ~ normal(0,1);
    }
  }
  for (g in 1:N_cause) {
  ...
  if(inference==1){
    for(g in 1:N_cause){
      for(i in 1:N_sex){
        for(j in 1:N_region){
          target += poisson_log_lpmf(deaths[g,i,j,] | mu0[g] + beta_sex[g,i] + beta_region[g,j] +
                                                  f_week[g,1:N] +
                                                  f_year[g,1:N] +
...
generated quantities {
  ...
  {//define variables eta inside brackets so that they are not saved
    ...
    for(g in 1:N_cause) {
      for(i in 1:N_sex){
        for(j in 1:N_region){
          mu_all[g,i,j] = mu0[g] +beta_sex[g,i] + beta_region[g,j]+ f_week[g] + f_year[g] + log(n_pop_all[i,j]);
        }
      }
      for(i in 1:N_all){
  ...
  matrix[N_cause,N_cause] Sigma = rho_chol * rho_chol';
}
