...
\begin{frame}{Result 3: lags turn pattern into mechanism}
  \begin{columns}[t]
    \begin{column}{0.45\linewidth}
      ...
      \end{itemize}
      \vspace{0.3em}
      {\footnotesize\punch{Why the lag matters:} miscoding a certificate is
      \emph{contemporaneous} --- it cannot produce a correlation peaking weeks
      later. The lag is what separates a coding artefact from a real
...
\begin{frame}{Backup: data and cause groups}
  \begin{itemize}\compresslist
    ...
    \item Immediate, underlying and contributing causes (ICD-10), grouped into
          9 categories: cardiovascular, mental/neurological, respiratory,
          infectious, cancer, external, suicide, COVID-19, other.
...
\begin{frame}{Backup: are the correlations just a coding artefact?}
  \textbf{The objection:} when a certificate lists both COVID-19 and
  cardiovascular involvement, only one becomes the underlying cause. Competition
  for the same certificate correlates the two causes \emph{by definition}.
  \vspace{0.4em}
  \begin{itemize}\compresslist
    \item \textbf{Timing.} Miscoding acts at lag $0$. The observed correlations
          peak at \textbf{1--3 weeks} --- a coding rule cannot generate that.
    \item \textbf{Sign.} Competition for one certificate would push causes in
          \emph{opposite} directions; several correlations here are positive.
    \item \textbf{Independent check.} The cancer result uses multiple-cause data,
          which does not depend on the underlying-cause assignment at all.
  \end{itemize}
  \vspace{0.3em}
  \textbf{Conceded:} contemporaneous misclassification is \emph{not} fully
  separable, and we report it as one of the four mechanisms rather than
...
\begin{frame}{Backup: is this just harvesting?}
  \begin{itemize}\compresslist
    \item Displacement is one of four mechanisms, and the model separates it
      ...
      \begin{itemize}\compresslist
        \item under-ascertainment $\Rightarrow$ excess in another cause
              \emph{concurrent} with COVID-19 (lag $\approx 0$--3 weeks),
        \item displacement $\Rightarrow$ deficit \emph{after} the wave.
      \end{itemize}
    \item The cancer result is checked independently with multiple-cause data,
          which does not rely on the excess model at all.
