...
%%% 1. THE PROBLEM -- framing only, no results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ~0:50
\begin{frame}{The interpretation problem}
  A pandemic moves cause-specific mortality in four ways at once:
  \begin{itemize}\compresslist
    \item direct deaths from SARS-CoV-2,
    \item deaths from other causes triggered by infection,
    \item displacement of deaths across time \emph{and} across causes,
    \item indirect effects of control measures and healthcare disruption.
  \end{itemize}
  \vfill
  \begin{center}
    So when deaths from a given cause fall below expectation,\\
    \punch{did those deaths not happen --- or were they counted elsewhere?}
  \end{center}
  \vfill
  \begin{itemize}\compresslist
    \item Single-cause excess mortality cannot tell these apart: each cause is
          modelled as if the others did not exist.
    \item \textbf{Aim:} estimate excess mortality for all causes jointly, and use
          the dependence between causes to identify what drove the disruptions.
  \end{itemize}
\end{frame}

%%% 2. THE HEADLINE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ~1:40
\begin{frame}{Result 1: all-cause mortality net of COVID-19}
  \begin{columns}[t]
    ...
    \begin{column}{0.43\linewidth}
      ...
      \begin{itemize}\compresslist
        ...
        \item \punch{Excluding COVID-19 deaths, Switzerland had fewer deaths
              than expected in 2020--21.}
      \end{itemize}
      \vspace{0.5em}
      \begin{center}
