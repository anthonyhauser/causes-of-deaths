...
fig4_supp = corr_post_res_df %>% 
  ...
  dplyr::mutate(not_cross0 = any(corr_lwb>0 | corr_upb<0),
                lag_peak = lag[which.max(abs(corr_mean))],
                peak = corr_mean[which.max(abs(corr_mean))]) %>% ungroup() %>% 
  dplyr::mutate(lag_peak = if_else(not_cross0,lag_peak,NA),
                peak = if_else(not_cross0,peak,NA)) %>% 
...
fig4 = corr_post_res_df %>% 
  dplyr::mutate(y = factor(y,levels=causes2_df$cod_group[c(1,2,3)],#put in the right order
                                   labels=causes2_df$cod_group_label[c(1,2,3)])) %>% 
